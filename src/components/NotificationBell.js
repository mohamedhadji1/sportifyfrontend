  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('Could not fetch team invitations:', inviteErr.response?.status || inviteErr.message);
        setNotifications([]);
      }
      ...
      try {
        ...
        console.warn('Could not fetch join requests:', joinErr.response?.status || joinErr.message);
        setJoinRequests([]);
      }
      ...
      try {
        ...
        console.warn('Could not fetch team offers:', offerErr.response?.status || offerErr.message);
        setTeamOffers([]);
      }
      ...
      if (user.role === 'Manager' || user.role === 'Admin') {
        try {
          ...
          console.warn('Could not fetch booking notifications:', bookingErr.response?.status || bookingErr.message);
