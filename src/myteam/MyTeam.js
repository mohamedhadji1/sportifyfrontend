              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {team.members.map((member) => (
                  <div key={member._id} className="bg-neutral-700 rounded-lg p-4 flex flex-col items-center text-center shadow-md hover:shadow-sky-500/20 transition-shadow duration-300">
                    <img 
                      src={member.profileImage ? `${process.env.REACT_APP_API_URL || process.env.REACT_APP_AUTH_SERVICE_URL || 'https://sportifyauth.onrender.com/api'}${member.profileImage}` : 'https://i.pravatar.cc/150?u=a042581f4e29026704d'} 
                      alt={member.fullName} 
